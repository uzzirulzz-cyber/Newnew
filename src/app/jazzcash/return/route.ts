...
/**
 * GET/POST /jazzcash/return
 *
 * JazzCash LIVE payment return URL.
 * This is the URL configured in Zapier: https://playbeat.digital/jazzcash/return
...
async function handleReturn(params: Record<string, string>, requestUrl: string) {
  ...
  if (billRef) {
    try {
      ...
      console.error("[jazzcash/return] DB error:", e);
    }
  }
  ...
  const redirectUrl = new URL(
    `/?payment=${statusParam}&ref=${txnRef}&msg=${encodeURIComponent(message)}`,
    origin
  );
